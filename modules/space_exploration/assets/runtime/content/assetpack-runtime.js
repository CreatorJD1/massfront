...
;
/* ============================================================================
   ASSET PACKS — high-detail content added after the base install
   ----------------------------------------------------------------------------
   The installer carries the complete current game: Galactic Exploration, its
   runtime models, voices and the available score. Future regions, campaigns and
   higher-detail art can grow into multiple-gigabyte packs without turning every
   base update into another multi-gigabyte APK.

   Startup-delivery packs fetch in the background after launch:
   ...
   THREE RULES keep that background delivery safe:
     * Base play never waits for a pack. Failure or no connection leaves the
       installed game playable and the last verified pack version mounted.
     * A remote manifest may request automatic startup delivery only in format
       2, where every bounded chunk and complete file has a SHA-256 identity.
     * Never re-download verified bytes. Completed chunks survive process death,
       and one atomic pointer exposes the new dependency set only after all of it
       verifies.
   ============================================================================ */
...
const PACK_WRITER_LOCK = 'massfront-content-writer-v1';
let packWriterDepth=0;
let packEndpointBlocked=false;
function packNetworkAllowed(){
  if(packEndpointBlocked)return false;
  if(typeof netAllowed==='function')return netAllowed();
  try{if(localStorage.getItem('massfront_offline')==='1')return false;}catch(e){}
  return typeof navigator==='undefined'||navigator.onLine!==false;
}
function packAutomaticNetworkPolicy(){
  const connection=typeof navigator!=='undefined'&&
    (navigator.connection||navigator.mozConnection||navigator.webkitConnection);
  if(!connection)return {ok:true};
  if(connection.saveData===true)return {ok:false,reason:'save-data',
    message:'Automatic content paused because Data Saver is enabled — use Install or Retry to download now'};
  const cost=String(connection.cost||'').toLowerCase();
  if(connection.metered===true||cost==='metered')return {ok:false,reason:'metered-network',
    message:'Automatic content paused on a metered connection — use Install or Retry to download now'};
  const effectiveType=String(connection.effectiveType||'').toLowerCase();
  const downlink=Number(connection.downlink);
  if(['slow-2g','2g','3g'].includes(effectiveType)||(Number.isFinite(downlink)&&downlink>0&&downlink<=1.5))
    return {ok:false,reason:'slow-network',
      message:'Automatic content paused on this slow connection — use Install or Retry to download now'};
  return {ok:true};
}
function packDeferAutomaticNetwork(policy){
  PACK.state='waiting';PACK.err=policy.message;PACK.currentPack='';PACK.currentFile='';
  packEmitState();if(typeof packRenderBar==='function')packRenderBar();
  return {ok:false,reason:policy.reason,automatic:true};
}
async function packWithWriter(action,automatic){
  const locks=typeof navigator!=='undefined'&&navigator.locks;
  if(!locks||typeof locks.request!=='function'){
    // Preserve legacy manual installation, but do not promise cross-page
    // automatic ownership without the browser's lifetime-bound mutex.
    if(automatic){PACK.state='error';PACK.err='Automatic content requires secure browser storage locks';packEmitState();return {ok:false,reason:'lock-unavailable'};}
    return action();
  }
  return locks.request(PACK_WRITER_LOCK,{mode:'exclusive',ifAvailable:true},async lock=>{
    if(!lock){PACK.state='waiting';PACK.err='Content is continuing in another game view';packEmitState();return {ok:false,reason:'writer-busy'};}
    packWriterDepth++;
    try{await packLoadCachedIndex();return await action();}
    finally{packWriterDepth--;}
  });
}
const packRequests=new Set();
async function packFetch(url,options){
  if(!packNetworkAllowed())throw new Error('offline');
  if(typeof AbortController==='undefined')return fetch(url,options);
  const abort=new AbortController();packRequests.add(abort);
  let timer;
  const arm=()=>{clearTimeout(timer);timer=setTimeout(()=>abort.abort(),60000);};
  const finish=()=>{clearTimeout(timer);packRequests.delete(abort);};
  arm();
  try{
    const response=await fetch(url,{...options,signal:abort.signal});
    if(!response.body||typeof ReadableStream==='undefined'||typeof Response==='undefined'){finish();return response;}
    const reader=response.body.getReader();
    // Timeout idle reads, not the duration of a progressing large download.
    const body=new ReadableStream({
      async pull(controller){
        arm();
        try{const row=await reader.read();if(row.done){finish();controller.close();}else{arm();controller.enqueue(row.value);}}
        catch(error){finish();controller.error(error);}
      },
      cancel(reason){abort.abort();finish();return reader.cancel(reason);}
    });
...
function packEndpoint(){
  const explicit=typeof UPDATE_URL==='string'&&UPDATE_URL||typeof window!=='undefined'&&window.MASSFRONT_UPDATE_URL;
  const locationInfo=typeof location!=='undefined'?location:null;
  const validate=value=>{
    try{
      const parsed=new URL(value,locationInfo&&locationInfo.href);
      if(parsed.username||parsed.password||parsed.search||parsed.hash)return '';
      if(parsed.protocol!=='https:'&&!(parsed.protocol==='http:'&&['127.0.0.1','localhost','[::1]'].includes(parsed.hostname)))return '';
      return parsed.href.replace(/\/$/,'');
    }catch(e){return '';}
  };
  if(explicit){
    const raw=String(explicit).replace(/\/update(?:-preview)?\.json.*$/, '/');
    // Legacy non-browser contract harnesses have no location/sessionStorage.
    if(!locationInfo)return raw.replace(/\/$/,'');
    const base=validate(raw);
    if(!base){packEndpointBlocked=true;return '';}
    packEndpointBlocked=false;
    try{sessionStorage.setItem('massfront_content_endpoint_v1',JSON.stringify({base,origin:locationInfo.origin,savedAt:Date.now()}));}catch(e){}
    return base;
  }
  // Only the standalone document consumes this handoff. The base updater still
  // resolves its own channel and is never overridden by yesterday's download.
  if(typeof UPDATE_URL==='undefined'&&locationInfo){
    try{
      const raw=sessionStorage.getItem('massfront_content_endpoint_v1');
      if(raw){
        const row=JSON.parse(raw),age=Date.now()-Number(row.savedAt);
        const base=validate(row.base);
        if(!base||row.origin!==locationInfo.origin||!Number.isFinite(age)||age<0||age>86400000){packEndpointBlocked=true;return '';}
        packEndpointBlocked=false;return base;
      }
    }catch(e){packEndpointBlocked=true;}
  }
  return '';
}
...
function packSnapshot(){
  return {
    state:PACK.state,busy:PACK.busy,got:PACK.got,total:PACK.total,
    percent:PACK.total?Math.min(100,Math.round(PACK.got/PACK.total*100)):0,
    ids:[...PACK.ids],currentPack:PACK.currentPack,currentFile:PACK.currentFile,
    error:PACK.err,storage:PACK.storage
  };
}
function packEmitState(){
  if(typeof window==='undefined'||typeof window.dispatchEvent!=='function'||typeof CustomEvent!=='function') return;
  try{window.dispatchEvent(new CustomEvent('massfront:assetpack-state',{detail:packSnapshot()}));}catch(e){}
}
...
async function packMergeOriginCatalog(merged){
  try{
    ...
    const r=await packFetch(PACK_ORIGIN_BASE+'/packs.json'+PACK_ORIGIN_QUERY+'&t='+Date.now(),{cache:'no-store'});
...
async function packLoadIndex(force=false){
  if(!force&&PACK.busy&&PACK.idx) return PACK.idx;
  const cached=await packLoadCachedIndex();
  if(!packNetworkAllowed()) return cached;
  ...
  try{
    ...
    let j=null;
    if(base){
      const r = await packFetch(base + '/packs.json?t=' + Date.now(), {cache:'no-store'});
    ...
    // Active-pointer adoption is also a writer. Read-only catalog callers
    // cannot overwrite another page's newly promoted dependency generation.
    if(!force&&(packWriterDepth>0||!(typeof navigator!=='undefined'&&navigator.locks)))try{await packAdoptInstalledOffers();}catch(e){}
    return PACK.idx;
  }catch(e){
...
function packNormalizeIndex(index){
  ...
  for(const [id,raw] of Object.entries(index.packs)){
    ...
    const delivery=raw.delivery===undefined?(raw.startup===true?'startup':'manual'):String(raw.delivery).toLowerCase();
    if(!['base','manual','startup'].includes(delivery)) throw new Error('delivery');
    if(raw.startup!==undefined&&typeof raw.startup!=='boolean') throw new Error('startup');
    if(raw.startup===true&&delivery!=='startup') throw new Error('delivery');
    ...
    if(delivery==='startup'&&(format<2||!files.length)) throw new Error('startup-format');
    if(raw.bytes!==undefined&&Number(raw.bytes)!==bytes) throw new Error('pack-bytes');
    out[id]={...raw,id,format,chunkSize,bytes,files,dependencies,delivery,startup:delivery==='startup'};
  }
...
async function packFetchChunk(base,pack,file,chunk,packMeta){
  ...
  const response=await packFetch(packFileUrl(base,pack,file,packMeta),{
...
function packDependencyOrder(pack,index=PACK.idx){
  return packDependencyOrderMany([pack],index);
}
function packDependencyOrderMany(packs,index=PACK.idx){
  const order=[],seen=new Set();
  function add(id){if(seen.has(id)) return;seen.add(id);for(const dep of index[id].dependencies) add(dep);order.push(id);}
...
async function packPlan(pack,{forceHash=false,index=PACK.idx}={}){
  return packPlanMany([pack],{forceHash,index});
}
async function packPlanMany(packs,{forceHash=false,index=PACK.idx}={}){
  const order=packDependencyOrderMany(packs,index),byPack=[],files=[];
  let bytes=0,storedBytes=0;
  for(const id of order){
...
async function packInstallMany(packs,options={}){
  if(PACK.busy)return {ok:false,reason:'busy'};
  PACK.busy=true;PACK.state='downloading';PACK.err='';
  try{return await packWithWriter(()=>{PACK.busy=false;return packInstallManyOwned(packs,options);},!!options.automatic);}
  finally{PACK.busy=false;packEmitState();}
}
async function packInstallManyOwned(packs,options={}){
  const requested=[...new Set(Array.isArray(packs)?packs.map(String):[])];
  if(!requested.length||requested.some(pack=>!packValidId(pack))) return {ok:false,reason:'pack-id'};
  if(PACK.busy) return {ok:false,reason:'busy'};
  PACK.busy=true;PACK.state='downloading';PACK.err='';PACK.storage=null;
  PACK.ids=requested;PACK.currentPack='';PACK.currentFile='';PACK.got=0;PACK.total=0;
  packEmitState();
  try{
    ...
    const idx=packWriterDepth?await packLoadIndex(true):(PACK.idx||await packLoadIndex());
    if(!idx||requested.some(pack=>!idx[pack])){
      PACK.state='error';PACK.err='Optional content manifest is unavailable';packEmitState();
      return {ok:false,reason:'manifest'};
    }
    const offerIndex=idx,offerRaw=PACK.rawIndex;
    const plan=await packPlanMany(requested,{forceHash:!!options.repair,index:offerIndex});
    PACK.total=plan.storedBytes+plan.bytes;PACK.got=plan.storedBytes;packEmitState();
    if(!plan.files.length){
      const gc=await packPromotePlan(plan,offerIndex,offerRaw);
      PACK.state='ready';packEmitState();
      return {ok:true,pack:requested[0],ids:requested,packs:plan.order,installed:true,downloaded:0,gc};
    }
    if(options.automatic){
      const policy=packAutomaticNetworkPolicy();
      if(!policy.ok)return packDeferAutomaticNetwork(policy);
    }
    if(!packNetworkAllowed()){
      PACK.state='error';PACK.err='Optional content is unavailable while offline';
      return {ok:false,reason:'offline'};
    }
    const storage=await packRequestPersistence(await packStoragePreflight(requested.join(','),plan));PACK.storage=storage;
    if(!storage.ok){
    ...
    const journalKey=requested.length===1?packJournalKey(requested[0]):packJournalKey('batch');
    await packStorePut(PACK_META_STORE,journalKey,{
      state:'installing',packs:requested,dependencies:plan.order,bytes:plan.bytes,startedAt:Date.now()
    });
    for(const item of plan.byPack){
      for(const file of item.missing.files){
        PACK.currentPack=item.pack;PACK.currentFile=file.name;packEmitState();
        await packDownloadFile(base,item.pack,file,n=>{
          ...
          packEmitState();if(options.ui) packRenderBar();
        },item.meta);
        await packStorePut(PACK_META_STORE,journalKey,{
          state:'installing',packs:requested,dependencies:plan.order,bytes:plan.bytes,downloaded:PACK.got,
    ...
    const gc=await packPromotePlan(plan,offerIndex,offerRaw);
    await packStorePut(PACK_META_STORE,journalKey,{
      state:'ready',packs:requested,dependencies:plan.order,bytes:plan.bytes,completedAt:Date.now()
    });
    PACK.state='ready';PACK.currentPack='';PACK.currentFile='';packEmitState();
    return {ok:true,pack:requested[0],ids:requested,packs:plan.order,installed:true,downloaded:plan.bytes,storage,gc};
  }catch(e){
    PACK.state='error';PACK.err='Content download needs attention — the base game remains playable';packEmitState();
    try{await packStorePut(PACK_META_STORE,requested.length===1?packJournalKey(requested[0]):packJournalKey('batch'),{
      state:'failed',packs:requested,reason:e&&e.code||'download',updatedAt:Date.now()
    });}catch(ignore){}
    return {ok:false,reason:e&&e.code||'download',message:e&&e.message||String(e)};
  }finally{
    PACK.busy=false;PACK.currentPack='';PACK.currentFile='';packEmitState();
    if(options.ui) packRenderBar();
  }
}
async function packInstallPack(pack,options={}){return packInstallMany([pack],options);}
...
async function packList(){
  ...
  return [...ids].map(id=>{
    ...
    return {
      ...
      dependencies:Array.isArray(pack.dependencies)?[...pack.dependencies]:[],delivery:pack.delivery||'manual',
      startup:pack.delivery==='startup'
    };
  });
}
async function packStatus(pack,options={}){
  ...
  return {
    ...
    delivery:idx[pack].delivery||'manual',startup:idx[pack].delivery==='startup',
...
async function packRemove(pack,options={}){
  if(PACK.busy)return {ok:false,reason:'busy'};
  return packWithWriter(()=>packRemoveOwned(pack,options),false);
}
async function packRemoveOwned(pack,options={}){
...
const MASSFRONT_ASSET_PACKS=Object.freeze({
  ...
  installMany:(packs,options)=>packInstallMany(packs,options||{}),
  startup:async options=>{
    const idx=PACK.idx||await packLoadIndex();
    if(!idx) return {ok:false,reason:'manifest'};
    const ids=Object.keys(idx).filter(id=>idx[id].delivery==='startup');
    return ids.length?packInstallMany(ids,{...(options||{}),automatic:true}):{ok:true,ids:[],packs:[],installed:true,downloaded:0};
  },
  snapshot:packSnapshot,
  initializeStartup:initAssetPacks,
  retryStartup:()=>packScheduleStartup(0,true),
...
/* ---- UI -------------------------------------------------------------------
   ...
   quiet when there is nothing to say, but continuously explicit about what an
   automatic startup transfer is doing and how much remains. */
function packPanel(){
...
function packRenderBar(){
  ...
  if(PACK.state === 'downloading'){
    ...
    const current=PACK.currentPack&&PACK.idx&&PACK.idx[PACK.currentPack];
    txt.textContent = 'DOWNLOADING GAME CONTENT';
    sub.textContent = packBytes(PACK.got) + ' of ' + packBytes(PACK.total) + '  ·  ' + pct + '%';
    if(current&&current.label) sub.textContent+='  ·  '+String(current.label);
    bar.style.width = pct + '%'; btn.textContent = '…'; btn.disabled = true;
    el.style.display = '';
  } else if(PACK.state === 'offer'){
    txt.textContent = 'GAME CONTENT AVAILABLE';
    sub.textContent = packBytes(PACK.total) + ' — resumable background content';
    bar.style.width = '0%'; btn.textContent = 'DOWNLOAD'; btn.disabled = false;
    el.style.display = '';
  } else if(PACK.state === 'error'||PACK.state === 'waiting'){
    txt.textContent = PACK.state==='waiting'?'CONTENT CONTINUING IN ANOTHER VIEW':'CONTENT DOWNLOAD PAUSED';
...
/* Current builds carry both categories directly. Keep their old pack IDs only
   as a manual recovery path for an installed legacy slim shell; automatic work
   is selected entirely by delivery:'startup' in the signed/hash-bound catalog. */
const PACK_LEGACY_MANUAL = ['voice', 'music'];
...
async function packStartInner(manual){
  if(packStartupOptions.ui!==false)packPanel();
  const idx = PACK.idx || await packLoadIndex();
  if(!idx){ PACK.state = 'idle'; packRenderBar(); return {ok:false,reason:'manifest'}; }
  const startup=Object.keys(idx).filter(p=>idx[p].delivery==='startup');
  /* Voice/music remain a manual compatibility route for older slim builds.
     They are deliberately not startup targets: current packages carry both. */
  const want=startup.length?startup:(manual?PACK_LEGACY_MANUAL.filter(p=>idx[p]):[]);
  if(!want.length){ PACK.state = 'idle'; packRenderBar(); return {ok:true,ids:[]}; }
  let activeOne=false;
  for(const p of PACK_LEGACY_MANUAL){
  ...
  if(manual&&!startup.length) try{ localStorage.setItem(PACK_PREF, 'auto'); }catch(e){}
  const result=await packInstallMany(want,{ui:packStartupOptions.ui!==false,automatic:!manual});
  ...
  if(!result||!result.ok){
    if(PACK.state!=='waiting')PACK.state = 'error';
    if(!PACK.err)PACK.err = 'Content download needs attention — the base game remains playable';
    packRenderBar();
  }
  return result;
}

let packStartupTimer=0,packStartupAttempts=0,packStartupInited=false,packStartupPaused=false;
let packStartupOptions={ui:true};
function packScheduleStartup(delay=4000,reset=false){
  if(reset){packStartupAttempts=0;PACK.idx=null;packEndpoint();}
  clearTimeout(packStartupTimer);packStartupTimer=0;
  if(packStartupPaused||!packNetworkAllowed())return;
  packStartupTimer=setTimeout(async()=>{
    packStartupTimer=0;
    if(packStartupPaused||!packNetworkAllowed())return;
    let result;
    try{result=await packStart(false);}
    catch(error){result={ok:false,reason:'download'};PACK.state='error';PACK.err='Content connection interrupted — verified progress is saved';packEmitState();}
    if(result&&result.ok){packStartupAttempts=0;return;}
    const reason=result&&result.reason;
    if(['lock-unavailable','storage','offline','save-data','metered-network','slow-network'].includes(reason))return;
    if(!result||reason==='busy'||reason==='writer-busy'){packScheduleStartup(3000);return;}
    if(packStartupAttempts<5){packStartupAttempts++;packScheduleStartup(Math.min(30000,1000*2**packStartupAttempts));}
  },Math.max(0,delay));
}
function initAssetPacks(options={}){
  packEndpoint();
  packStartupOptions={...packStartupOptions,...options};
  if(packStartupOptions.ui!==false){packPanel();packRenderBar();}
  if(packStartupInited)return;
  packStartupInited=true;
  if(typeof window!=='undefined'&&typeof window.addEventListener==='function'){
    window.addEventListener('online',()=>packScheduleStartup(250,true));
    window.addEventListener('offline',()=>{clearTimeout(packStartupTimer);for(const abort of packRequests)abort.abort();});
    window.addEventListener('pagehide',()=>{packStartupPaused=true;clearTimeout(packStartupTimer);for(const abort of packRequests)abort.abort();});
    window.addEventListener('pageshow',()=>{packStartupPaused=false;packScheduleStartup(250,true);});
    window.addEventListener('storage',event=>{if(event.key==='massfront_offline')packScheduleStartup(250,true);});
  }
  packScheduleStartup();
}
...
/* Galactic Exploration ships in player APK/www from the signed runtime pack.
   ...
   read only for verified migration. A standalone HTML Blob does not provide
   the relative resource tree needed by ES modules/models. Native delivery
   instead stages an immutable directory through the existing same-origin
   Capacitor file route. That adapter never replaces the base WebView root. */
...
async function mfExplorationRemoteSpec(){
  let stub=null;
  try{
    // OTA inlines this small descriptor into trusted executable transport.
    // data: decoding needs no network, including during an offline relaunch.
    const r=await (EXP_PACK_STUB.startsWith('data:')?fetch(EXP_PACK_STUB):packFetch(EXP_PACK_STUB,{cache:'no-store'}));
  ...
  return {base,manifest,downloadQuery:q,schema:stub&&stub.schema,version:stub&&stub.version,
    manifestSha256:stub&&stub.manifestSha256,manifestBytes:stub&&stub.manifestBytes};
}
async function mfInstallExplorationPack(options={}){
  if(expPackBusy) return {ok:false,reason:'busy'};
  if(typeof indexedDB==='undefined') return {ok:false,reason:'no-idb'};
  expPackBusy=true;
  if(!options.automatic&&typeof toast==='function') toast('Galactic pack downloading...');
  try{
    const spec=await mfExplorationRemoteSpec();
    const bound=spec.schema==='MassfrontExplorationPackRemoteV2'
      &&/^[a-f0-9]{64}$/.test(spec.manifestSha256||'')&&Number.isSafeInteger(spec.manifestBytes)
      &&spec.manifestBytes>0&&spec.manifestBytes<=8*1024*1024;
    ...
    if(cached&&cached[EXP_PACK_ID]){
      ...
      if(ready.installed){
        if(bound&&typeof mfPrepareExplorationMount==='function'
          &&await packStoreGet(PACK_META_STORE,'exploration:manifest:'+spec.manifestSha256)){
          const mounted=await mfPrepareExplorationMount(spec);
          if(mounted.ok)return {...mounted,cached:true,pack:EXP_PACK_ID};
          if(!packNetworkAllowed())return mounted;
        }else if(!bound)return {ok:true,cached:true,openUrl:null,pack:EXP_PACK_ID};
      }
    }
    if(options.automatic){
      const policy=packAutomaticNetworkPolicy();
    ...
    if(!packNetworkAllowed()) return {ok:false,reason:'offline'};
    ...
    const mr=await packFetch(manUrl,{cache:'no-store'});
    if(!mr.ok) return {ok:false,reason:'manifest'};
    const manifestText=await mr.text();
    if(bound&&(new TextEncoder().encode(manifestText).length!==spec.manifestBytes
      ||await packHashBlob(new Blob([manifestText]))!==spec.manifestSha256))return {ok:false,reason:'manifest-integrity'};
    ...
    for(const entry of man.files){
      ...
      files.push({name:rel,size:Number(entry.bytes),sha256:match[1].toLowerCase(),...(bound?{chunks:entry.chunks}:{})});
    }
    ...
    const registered=await packRegisterPack(EXP_PACK_ID,{
      format:bound?2:1,label:'Galactic Exploration',bytes,chunkSize:PACK_DEFAULT_CHUNK_BYTES,
      baseUrl:spec.base,downloadQuery:spec.downloadQuery||'',files
    });
    if(bound)await packStorePut(PACK_META_STORE,'exploration:manifest:'+spec.manifestSha256,manifestText);
    ...
    const result=await packInstallPack(EXP_PACK_ID,{automatic:!!options.automatic});
    if(result.ok&&bound&&typeof mfPrepareExplorationMount==='function'){
      const mounted=await mfPrepareExplorationMount(spec);return {...mounted,cached:!!result.ok,pack:EXP_PACK_ID};
    }
