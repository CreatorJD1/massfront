...
export const WORLD_KIT_FAMILIES = Object.freeze([
  ...
  { id: 'mf-building-hs-v1', label: 'Hard-surface buildings', expected: 33 },
  { id: 'mf-cityforms-kit-v1', label: 'City forms & industrial landmarks', expected: 68 },
  { id: 'mf-superstructure-v1', label: 'Megastructures & fortifications', expected: 56 },
]);
...
const SPLINE_METADATA_BLOCKED_IDS = Object.freeze([]);
...
function buildSplineExports() {
  ...
  if (modelPaths.length !== 7) throw new Error(`Expected 7 retained Spline exports, found ${modelPaths.length}`);
  return modelPaths.map((modelPath) => {
...
function buildHtml(catalog) {
  ...
  const repairSection = section('unfinished-models', 'Unfinished models / repair required', repairLocked,
    'Empty. The creator visually accepted the former seven hard-surface buildings as good on 2026-08-31; they are processing-eligible.');
  ...
  const roadSection = section('road-qa', 'Road QA review exports', catalog.roadQa,
    '31 LOD, collision, nav, reference and junction files kept separate from the 320 report-authoritative world-kit modules.');
  ...
  <div class="warning"><strong>Everything shown is runtime inactive and unregistered.</strong> No hard-surface buildings remain repair-locked; Spline exports remain render-gated.</div>
...
export function buildCatalog() {
  ...
  return {
    ...
    exclusions: [
      ...
      { label: 'Discarded Stage 10 pack failures', entries: [
      ...
      { label: 'Discarded Stage 10 Spline exclusions', entries: [
