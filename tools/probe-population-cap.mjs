...
try{
  ...
  const repetitions=await page.evaluate(()=>{
    ...
    curMap='vanguard';curTheme='verdant';terrainTex=buildTerrain(curTheme);builtTheme=curTheme;builtMap=curMap;
    ...
    const run=repetition=>{
      ...
      const ordinaryAccepted=AI.bases.map(B=>fill(1,B.slot,600,B).length);
      ...
      const sameFaction1v3={heroes,ordinaryAccepted,used:populationUsedFor(1),cap:populationCapFor(1),ceiling:populationTeamCeiling(1),seatUsed:[0,1,2].map(populationUsedForCommander),furtherBySeat:AI.bases.map(B=>populationCanSpawn(ordinary,1,B.slot,B.x,B.y)),fullTypes};
      ...
      const seat1Ordinary=fill(1,1,600,AI.bases[1]);
      const afterDeath={used:populationUsedFor(1),seat1Ordinary:seat1Ordinary.length,ordinaryAllowed:populationCanSpawn(ordinary,1,1,RP.x,RP.y),heroAllowed:populationCanSpawn(hero,1,1,RP.x,RP.y)};
      ...
      return {repetition,seed:SEED,constant:FACTION_POP_CAP,sameFaction1v3,factory,independentFactions,invalidSlots,slot3,reservations,capFallback,cargoRecount:{cargoHero,cargoBeforeRecount,cargoAfterRecount,cargoAfterRelease}};
    };return [run(1),run(2)];
  });
  ...
  const checks={
    ...
    sameTeam1v3Gets500PerSeat:first.sameFaction1v3.heroes.every(id=>id>=0)&&first.sameFaction1v3.ordinaryAccepted.every(n=>n===499)&&first.sameFaction1v3.used===1500&&first.sameFaction1v3.cap===1500&&first.sameFaction1v3.ceiling===1500&&first.sameFaction1v3.seatUsed.every(n=>n===500)&&first.sameFaction1v3.furtherBySeat.every(v=>!v),
    everyTypeRejectedAtParticipantCap:first.sameFaction1v3.fullTypes.length>0&&first.sameFaction1v3.fullTypes.every(r=>!r.allowed&&r.spawned<0),
    ...
    hudReadsPlayerSeat500:first.independentFactions.hud.used===500&&first.independentFactions.hud.cap===500,
    ...
    invalidTeamCapFailsClosed:first.capFallback.invalidCap===0&&first.capFallback.invalidCeiling===0&&Number.isFinite(first.capFallback.neutralCap)&&first.capFallback.neutralCap>=0,
    ...
    eachCommanderRespawnReserved:first.reservations.ordinaryBeforeHeroes===499&&!first.reservations.blockedOrdinary&&first.reservations.restoredHeroes.every(id=>id>=0)&&first.reservations.usedAfterHeroes===502&&first.reservations.afterDeath.seat1Ordinary===499&&first.reservations.afterDeath.used===1000&&!first.reservations.afterDeath.ordinaryAllowed&&first.reservations.afterDeath.heroAllowed&&first.reservations.afterDeath.heroRespawn>=0&&first.reservations.afterDeath.usedAfterRespawn===1001,
    ...
    factoryPausesBeforePayment:first.factory.before.used===1500&&first.factory.after.used===1500&&first.factory.queueRetained&&first.factory.before.mass===first.factory.after.mass&&first.factory.before.energy===first.factory.after.energy&&first.factory.after.prodT<=first.factory.before.prodT,
  ...
  const pass=Object.values(checks).every(Boolean),report={schema:'MassfrontParticipantPopulationProbeV5',startedUtc,finishedUtc:new Date().toISOString(),result:pass?'PASS':'FAIL',provenance:{start:startSource,end:endSource,sourceLines:lines},runtime:{url:server.url,port:server.port,gpu,blockedExternalCount:blockedExternal.length},servedSources:served,servedMismatch,runtimeErrors,consoleErrors,repetitions,checks,interpretation:{scope:'500 per participant/Commander seat; team totals are aggregate diagnostics',reservation:'One place for the absent Commander in that same seat',saveCompatibility:'No serialized field changed; transient ledgers recount existing unit/team/commander data.'}};
  await writeFile(join(output,'report.json'),`${JSON.stringify(report,null,2)}\n`);
  const md=['# MASSFRONT participant population admission probe','',`- Result: **${report.result}**`,`- HEAD: \`${startSource.head}\``,`- Source-set SHA-256: \`${startSource.sourceSetSha256}\``,`- GPU: ${gpu.renderer} (${gpu.vendor})`,'','## Measured','',`- Same-team 1v3: heroes=${first.sameFaction1v3.heroes.length}, ordinary per seat=${first.sameFaction1v3.ordinaryAccepted.join('/')}, total=${first.sameFaction1v3.used}/${first.sameFaction1v3.cap}, seats=${first.sameFaction1v3.seatUsed.join('/')}`,`- Independent participants: player=${first.independentFactions.player.used}/500, enemy=${first.independentFactions.enemy.used}/500`,`- Missing Commanders: seat-0 ordinary=${first.reservations.ordinaryBeforeHeroes}, restored heroes=${first.reservations.restoredHeroes.length}, aggregate=${first.reservations.usedAfterHeroes}`,`- Invalid slot 99: index=${first.invalidSlots.index99}, resolved=${first.invalidSlots.resolved99}, spawned=${first.invalidSlots.spawned99}`,`- Fourth AI slot 3: index=${first.slot3.index}, used=${first.slot3.used}`,`- Cargo count before/recount/release: ${first.cargoRecount.cargoBeforeRecount.faction}/${first.cargoRecount.cargoAfterRecount.faction}/${first.cargoRecount.cargoAfterRelease.faction}`,`- Factory mass before/after: ${first.factory.before.mass}/${first.factory.after.mass}; queue retained=${first.factory.queueRetained}`,`- Repeat hashes: \`${first.repeatSha256}\` / \`${second.repeatSha256}\``,'','## Checks','',...Object.entries(checks).map(([n,v])=>`- ${v?'PASS':'FAIL'} — ${n}`),'','## Source anchors','',...Object.entries(lines).map(([n,v])=>`- ${n}: ${v}`)].join('\n');
